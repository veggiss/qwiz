$(document).ready(function(){function r(n){let t=new FormData;return t.set("image",n),new Promise(function(n,i){axios.post("/api/uploadImage",t,{headers:{"Content-Type":"multipart/form-data"}}).then(function(t){n(t.data)}).catch(function(n){i(n)})})}function e(){$(".collapseQuizForm").each(function(n){this.innerHTML="Question "+(n+1)})}function o(){if(t()>1&&$("#quizForm")[0].checkValidity()){let t="/api/create",n={topic:$("#quizTitle").val(),category:$("#quizCategory").val(),description:$("#quizDescription").val(),imagePath:$("#quizImgPrev").attr("src"),difficulty:$("#quizDifficulty").children("option:selected").val(),questions:[]};$(".quizForm").each(function(){let i=f([$(this).find(".questionType")[0].selectedIndex]),t=$(this).find(".alternative"),r=i==="multiple_choice"?JSON.stringify([t[0].value,t[1].value,t[2].value,t[3].value]):null,u={correctAnswer:$(this).find(".correctAnswer").children("option:selected").val(),questionText:$(this).find(".questionText")[0].value,questionType:i,imagePath:$(this).find(".questionImgPrev").attr("src"),alternatives:r,difficulty:$(this).find(".questionDifficulty").children("option:selected").val().toLowerCase()};n.questions.push(u)});i()&&(n.id=model.id,n.ownerId=model.ownerId,t="/api/update");axios.post(t,JSON.stringify(n),{headers:{"Content-Type":"application/json"}}).then(function(n){n.status===200&&(window.location.href="/Profile")})}else $(".alert").show().delay(2e3).fadeOut()}function s(){if(i()){let n=model;$("#btnCreateQuiz").text("Update Quiz");$("#quizTitle").val(n.topic);$("#quizCategory").val(n.category);$("#quizDescription").val(n.description);$("#quizImgPrev").attr("src",n.imagePath);n.questions.forEach(n=>{let t=$(u()),r=$(t).find(".alternative");$(t).find(".questionText").val(n.questionText);$(t).find(".questionDifficulty").val(n.difficulty);$(t).find(".questionImgPrev").attr("src",n.imagePath);let i=JSON.parse(n.alternatives);i!=null&&i.forEach((n,t)=>{$(r[t]).val(n)});$("#quizField").append(t);$(t).find(".questionType").val(n.questionType);$(t).find(".questionType").trigger("change");$(t).find(".correctAnswer").val(n.correctAnswer)});$("#deleteDiv").show();$("#btnDeleteQuiz").click(function(){axios.delete("/api/delete?id="+n.id).then(function(n){n.status===200&&(window.location.href="/Profile")})})}}function u(){return`
                ...
                </div>`}let f=n=>["multiple_choice","true_false"][n],t=()=>$(".quizForm").length+1,n=n=>$(n).closest(".quizForm"),i=()=>model!==null;$("#btnAddQuizForm").click(function(){$("#quizField").append(u())});$("#btnCreateQuiz").click(function(){o()});$(".alert .close").click(function(){$(".alert").hide()});$(document).on("change",".questionType",function(t){let i=n(this).find(".correctAnswer"),r=n(this).find(".alternatives");switch(t.currentTarget.selectedIndex){case 0:r.show();i.empty();r.find("input").each(function(){$(this).attr("required",!0)});["A","B","C","D"].forEach(n=>{i.append($("<option><\/option>").text(n))});break;case 1:r.hide();i.empty();r.find("input").each(function(){$(this).attr("required",!1)});[{text:"True",value:!0},{text:"False",value:!1}].forEach(n=>{i.append($("<option><\/option>").attr("value",n.value).text(n.text))});break;default:console.error("Not a valid question type!")}});$(document).on("click","button.removeQuizForm",function(){n(this).remove();e()});$(document).on("click","button.collapseQuizForm",function(){n(this).find(".collapse").slideToggle("slow","swing")});$(document).on("change","input.questionImg",function(){r(this.files[0]).then(t=>{n(this).find(".questionImgPrev").attr("src",t)}).catch(n=>console.log(n))});$("#quizImg").change(function(){r(this.files[0]).then(n=>{$("#quizImgPrev").attr("src",n)}).catch(n=>console.log(n))});s()});