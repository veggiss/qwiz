$(document).ready(function(){function r(n){let t=new FormData;return t.set("image",n),new Promise(function(n,i){axios.post("/api/uploadImage",t,{headers:{"Content-Type":"multipart/form-data"}}).then(function(t){n(t.data)}).catch(function(n){i(n)})})}function e(){$(".collapseQuizForm").each(function(n){this.innerHTML="Question "+(n+1)})}function o(){if(t()>1&&$("#quizForm")[0].checkValidity()){let t="/api/create",n={topic:$("#quizTitle").val(),category:$("#quizCategory").val(),description:$("#quizDescription").val(),imagePath:$("#quizImgPrev").attr("src"),difficulty:$("#quizDifficulty").children("option:selected").val(),questions:[]};$(".quizForm").each(function(){let i=f([$(this).find(".questionType")[0].selectedIndex]),t=$(this).find(".alternative"),u=i==="multiple_choice"?[[t[0].value,t[1].value,t[2].value,t[3].value]]:null,r=$(this).find(".correctAlternative").children("option:selected")[0],e=i==="true_false"?r.innerHTML:$(this).find(`[name=${r.value}]`)[0].value,o={correctAnswer:e,correctAlternative:r.name,questionText:$(this).find(".questionText")[0].value,questionType:i,imagePath:$(this).find(".questionImgPrev").attr("src"),alternatives:u,difficulty:$(this).find(".questionDifficulty").children("option:selected").val().toLowerCase()};n.questions.push(o)});i()&&(n.id=model.id,n.ownerId=model.ownerId,t="/api/update");console.log(n)}else $(".alert").show().delay(2e3).fadeOut()}function s(){if(i()){let n=model;$("#btnCreateQuiz").text("Update Quiz");$("#quizTitle").val(n.topic);$("#quizCategory").val(n.category);$("#quizDescription").val(n.description);$("#quizImgPrev").attr("src",n.imagePath);n.questions.forEach(n=>{let t=$(u()),r=$(t).find(".alternative");$(t).find(".questionText").val(n.questionText);$(t).find(".questionDifficulty").val(n.difficulty);$(t).find(".questionImgPrev").attr("src",n.imagePath);let i=JSON.parse(n.alternatives);i!=null&&i.forEach((n,t)=>{$(r[t]).val(n)});$("#quizField").append(t);$(t).find(".questionType").val(n.questionType);$(t).find(".questionType").trigger("change");$(t).find(".correctAlternative").val(n.correctAlternative)});$("#deleteDiv").show();$("#btnDeleteQuiz").click(function(){axios.delete("/api/delete?id="+n.id).then(function(n){n.status===200&&(window.location.href="/Profile")})})}}function u(){return`
